...
mapIndex <- which( MOM0 > 15.0 ); MOM0[MOM0 < 15.0] <- NA #flagIndex <- which( MOM0 < 15.0 ); MOM0[flagIndex] <- NA
MOM1 <- matrix(nrow=nrow(FITS$imDat), ncol=ncol(FITS$imDat))
for(index in mapIndex){
	row_index <- (index - 1) %% length(ax2) + 1
	col_index <- ceiling( index / length(ax2) )
...
image(-ax1+mapCenter[1], ax2-mapCenter[2] ,MOM1, xlab = xlab, ylab = ylab, main=fileName, col = rainbow(128)); points(0,0, pch=3)
DF <- data.frame( x=ax1[(mapIndex - 1) %% length(ax2) + 1] - mapCenter[1], y=ax2[ceiling( mapIndex / length(ax2) )] - mapCenter[2], z=MOM1[mapIndex], w=MOM0[mapIndex])
