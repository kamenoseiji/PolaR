...
if( length(WG$mjdSec) > 3 ){
    ...
    WG <- data.frame(
        mjdSec=c(min(WG$mjdSec)-1000, WG$mjdSec, max(WG$mjdSec)+1000),
    ...
    delay00Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=WG$delay00))
    delay01Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=WG$delay01))
    Re00Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=Re(WG$Vis00)))
    Im00Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=Im(WG$Vis00)))
    Re01Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=Re(WG$Vis01)))
    Im01Fit <- lm(formula = y ~ 1, data.frame(x=WG$mjdSec, y=Im(WG$Vis01)))
}
#
#-------- Initial Parameters
chNum <- dim(on_A00)[1]
