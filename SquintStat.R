...
SquintDF <- data.frame( dAZ = DF$Raz - DF$Laz, dEL = DF$Rel - DF$Lel, eAZ = sqrt( DF$Raze^2 + DF$Laze^2), eEL = sqrt( DF$Rele^2 + DF$Lele^2))
...
cat(text_sd); cat('\n')
pdf("Squint.pdf")
Xlim <- c( max(SquintDF$dAZ + SquintDF$eAZ), min(SquintDF$dAZ - SquintDF$eAZ) )
