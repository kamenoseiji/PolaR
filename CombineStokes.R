...
#lineFreq <- c(2.15, 2.4)
plotFreq <- c(1.6, 1.97) # range in MHz
lineFreq <- c(1.65, 1.9)
#plotFreq <- c(0.1, 0.5)
...
plotBunch <- 1
plot(bunch_vec(freq[plotRange],plotBunch)-0.5*plotBunch*chSep, bunch_vec(StokesI02[plotRange], plotBunch), xlab='Frequency [MHz]', ylab='Stokes I [K]', main='TMC-1 HC3N', type='s', lwd=0.1)
lines(freq[plotRange], predStokesI[plotRange], type='s', col='red', lwd=1)
...
plot( plotX, plotY , pch=20, ylim=c(-7e-2, 7e-2), xlab='Frequency [MHz]', ylab='Stokes V [K]', main='TMC-1 HC3N')
...
legend(min(freq[plotRange]), 0.07, legend=sprintf('Zeeman Shift = %5.1f ± %4.1f Hz', 10*fit[[1]][2], 10*summary(fit)[[4]][5]))
...
predStokesV <- predict(fitStokesI, (freq + 5e-6))$y - predict(fitStokesI, (freq - 5e-6))$y
plotBunch <- 1
plot( bunch_vec(freq[plotRange],plotBunch)-0.5*plotBunch*chSep, bunch_vec(StokesI13[plotRange], plotBunch), type='s', xlab='Frequency [MHz]', ylab='Stokes I [K]', main='TMC-1 CCS', lwd=0.1)
lines(freq[plotRange], predStokesI[plotRange], type='s', col='red', lwd=1)
...
plot( plotX, plotY , pch=20, ylim=c(-3e-2, 3e-2), xlab='Frequency [MHz]', ylab='Stokes V [K]', main='TMC-1 CCS')	# plot points
arrows( plotX, plotY - err, plotX, plotY + err, angle=90, length=0)
lines(freq[plotRange]-0.5*chSep, predStokesV[plotRange]*fit$coefficients[2], type='s', col='red')
